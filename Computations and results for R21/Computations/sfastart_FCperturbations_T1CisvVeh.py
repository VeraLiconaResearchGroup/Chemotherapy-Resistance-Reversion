...
##read in readout nodes, readout node attractor state file
    comparison_folder=Path("C:/Users/marazzi/Dropbox/JAX&CQM_StructureBasedControlTNBC/2.18.experiments/T1CisvVeh")
    ronfile=comparison_folder/'t1cisvveh_ron.txt'
    readout_nodes=open(ronfile).read().split('\n')

    ronssfile=comparison_folder/'t1cis_ss.txt'
...
##  read in full network nodes, and their initial conditions (normalized expression)
    ...
    attractor1_file=attractors_folder/'attractor_t1cis.txt'
    networkstatesfile=open(attractor1_file).read()
    j=0

##  read in FC nodes
    FC=open('FC3_nodes.txt').read().split('\n')


## read in ideal LogFC direction
    logFCfile=comparison_folder/'t1cisvveh_logFC'
...
##  if i is a readout node, save values to readout_dict, and compare values to logFC
        ...
        if (quantifier/num_readouts)>.78:
            timeout=True
            print(quantifier/num_readouts)
            print(quantifier)
            outputfile=open('SFAoutput_T1CisvVeh_FC3_5.txt','w+') #SFA results
...
##  create output file for FC configuratoin
            FCout=open('T1CisvVeh_FC3_5.txt','w+')
